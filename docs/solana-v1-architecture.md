...
Each city market is capped at 10,000 USDC liquidity, 8,000 USDC reserved payout exposure, and 500 USDC coverage per wallet. USDC has six decimals. Payout equals protected amount when the immutable threshold is met.
...
Winning protection claims transfer USDC from the market vault once. If NOAA cannot return final source data within seven days, the settlement signer records `DATA_UNAVAILABLE`; buyers may reclaim their premium once. At the immutable deadline—seven days of source grace plus 30 days of claims—the admin closes the market, moves fees to the fee vault, and LPs redeem all remaining assets pro rata.
