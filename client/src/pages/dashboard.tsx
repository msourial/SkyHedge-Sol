...
import { AGRICULTURAL_MARKETS, agriculturalMarketBySlug, millimetersToInches, type AgriculturalMarketSlug } from "../../../shared/agricultural-markets";
...
function Markets({ onSelect }: { onSelect: (id: MarketId) => void }) { return <section><div className="flex flex-wrap items-end justify-between gap-4"><div><SectionLabel>Agricultural rainfall indexes</SectionLabel><p className="max-w-2xl text-sm text-[var(--muted-foreground)]">USD protection previews for regional crop risk. All values remain pre-launch until a NOAA station is validated and USDC collateral is available.</p></div><Pill tone="amber">USD preview only</Pill></div><div className="mt-5 grid gap-4 lg:grid-cols-3">{MARKETS.map((market) => <Card key={market.id} hover className="relative overflow-hidden p-5"><div className="absolute right-0 top-0 h-16 w-16 border-b border-l border-[var(--signal)]/40" aria-hidden /><div className="flex items-start justify-between gap-3"><div><p className="sky-eyebrow">{market.region} · rainfall index</p><h2 className="sky-display mt-1 text-lg font-semibold">{market.city}</h2></div><Pill tone={market.evidenceStatus === "validated" ? "green" : "amber"}>{market.evidenceStatus === "validated" ? "Evidence validated" : "Researching evidence"}</Pill></div><p className="mt-3 text-xs leading-relaxed text-[var(--muted-foreground)]">{market.context}</p><p className="sky-mono mt-3 text-xs text-[var(--faint)]">Crop systems: {market.crops.join(" · ")}</p><div className="mt-5 grid grid-cols-3 gap-2"><Stat label="Index" value="Rainfall" /><Stat label="Unit" value="mm / in" /><Stat label="Expiry" value="Wk / Mo" /></div><p className="mt-4 text-xs text-[var(--warning)]">{market.station}</p><button className="sky-btn-primary mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2" onClick={() => onSelect(market.id)}>Explore index <ArrowRight className="h-4 w-4" /></button></Card>)}</div></section>; }
