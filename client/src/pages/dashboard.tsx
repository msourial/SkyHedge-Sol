...
function Evidence() {
  ...
  const weatherXm = useQuery({ queryKey: ["weatherxm", city], queryFn: () => api<{ source: "WeatherXM"; settlementEligible: false; location: { latitude: number; longitude: number }; agent: { baseUrl: string; freeEndpoint: string; paidEndpoints: string[]; paymentProtocol: string; priceUsdPerRequest: string }; status: string; message: string }>(`/api/weatherxm/${city}/latest`), retry: false });
