...
import { AGRICULTURAL_MARKETS, agriculturalMarketBySlug, millimetersToInches, searchAgriculturalMarkets, SUGGESTED_AGRICULTURAL_MARKET_SLUGS, type AgriculturalMarketSlug } from "../../../shared/agricultural-markets";

const MARKETS = AGRICULTURAL_MARKETS.map((market) => ({ id: market.slug, city: market.name, station: market.evidenceStatus === "validated" ? market.noaaStationId : "NOAA station validation in progress", crops: market.crops, region: market.region, context: market.agriculturalContext, evidenceStatus: market.evidenceStatus })) as Array<{ id: AgriculturalMarketSlug; city: string; station: string | null; crops: readonly string[]; region: string; context: string; evidenceStatus: "researching_evidence" | "validated" }>;
const TABS = [
...
function Protect(props: { marketId: MarketId; amount: string; setAmount: (value: string) => void; threshold: string; setThreshold: (value: string) => void; operator: "gte" | "lte"; setOperator: (value: "gte" | "lte") => void; start: string; setStart: (value: string) => void; end: string; setEnd: (value: string) => void; valid: boolean; requestQuote: () => void; quote?: Quote; loading: boolean; error: unknown }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><section className="sky-hero-card"><SectionLabel className="text-[var(--identity)]">Agricultural index / {market.city}</SectionLabel><h2 className="sky-display mt-2 text-2xl font-semibold">Measure rainfall, protect crop exposure.</h2><p className="mt-2 max-w-xl text-sm text-[var(--muted-foreground)]">This index measures cumulative liquid rainfall. {hasThreshold ? `${thresholdMm} mm equals ${millimetersToInches(thresholdMm)} in.` : "Millimetres are canonical; inches are a display conversion."}</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><Field label="Protection amount (USD preview)"><input className="sky-input" inputMode="decimal" value={props.amount} onChange={(event) => props.setAmount(event.target.value)} /></Field><Field label="Rainfall threshold (mm / in)"><input className="sky-input" inputMode="decimal" aria-describedby="threshold-unit-help" value={props.threshold} onChange={(event) => props.setThreshold(event.target.value)} /><p id="threshold-unit-help" aria-live="polite" className="mt-2 text-xs leading-relaxed text-[var(--muted-foreground)]">{hasThreshold ? `${thresholdMm} mm = ${millimetersToInches(thresholdMm)} in. Enter millimetres; inches are display-only.` : "Enter millimetres; inches are displayed here for reference."}</p></Field><Field label="Risk direction"><select className="sky-input" value={props.operator} onChange={(event) => props.setOperator(event.target.value as "gte" | "lte")}><option value="gte">Rainfall at or above threshold</option><option value="lte">Rainfall at or below threshold</option></select></Field><div className="grid grid-cols-2 gap-3"><Field label="Window start"><input className="sky-input" type="date" value={props.start} onChange={(event) => props.setStart(event.target.value)} /></Field><Field label="Window end"><input className="sky-input" type="date" value={props.end} onChange={(event) => props.setEnd(event.target.value)} /></Field></div></div><p role="alert" className="mt-4 text-sm text-[var(--warning)]">This market is researching its final NOAA evidence station. USD previews are non-binding and checkout is unavailable until evidence and USDC collateral are live.</p><button disabled className="sky-btn-primary mt-6 min-h-11 w-full sm:w-auto">Purchase unavailable during validation</button></section><QuotePanel quote={props.quote} error={props.error} /></div>;
}
...
function Evidence() {
  ...
  return <section className="space-y-5"><div><SectionLabel>Evidence sources</SectionLabel><p className="max-w-2xl text-sm text-[var(--muted-foreground)]">NOAA is the sole settlement source. WeatherXM is supplemental, read-only local context and never determines a payout.</p></div><Card className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="sky-display text-lg">WeatherXM live context</h2><p className="mt-1 text-xs text-[var(--muted-foreground)]">Nearest reporting station to the selected crop-belt location. This information is not settlement evidence.</p></div><select aria-label="WeatherXM market location" className="sky-input max-w-44" value={city} onChange={(event) => setCity(event.target.value as MarketId)}>{MARKETS.map((market) => <option key={market.id} value={market.id}>{market.city}</option>)}</select></div>{weatherXm.isLoading ? <p className="mt-4 text-sm text-[var(--muted-foreground)]">Requesting WeatherXM observation…</p> : weatherXm.isError ? <p role="alert" className="mt-4 text-sm text-[var(--warning)]">WeatherXM data is unavailable for this location. Add a server-side WeatherXM Pro API key, or no reporting station may be nearby. NOAA settlement is unchanged.</p> : weatherXm.data ? <div className="mt-4 grid gap-2 sm:grid-cols-3"><Stat label="Station" value={weatherXm.data.station.name ?? weatherXm.data.station.id.slice(0, 8)} accent="cyan" /><Stat label="Observed" value={new Date(weatherXm.data.observation.timestamp).toLocaleString()} /><Stat label="Quality" value={weatherXm.data.observation.dataQuality == null ? "Not reported" : `${(weatherXm.data.observation.dataQuality * 100).toFixed(0)}%`} /></div> : null}</Card>{evidence.isError ? <DeploymentNotice /> : !evidence.data?.rows.length ? <EmptyState icon={<FileCheck2 className="h-8 w-8" />} title="No finalized NOAA settlement evidence" hint="Evidence appears after a valid observation window is finalized on-chain." /> : <div className="space-y-3">{evidence.data.rows.map((item) => <Card key={item.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="sky-display text-base">{item.city.replaceAll("-", " ")}</h2><p className="sky-mono mt-1 text-xs text-[var(--faint)]">{item.windowStart} → {item.windowEnd}</p></div><Pill tone={item.verdict === "DATA_UNAVAILABLE" ? "red" : "green"}>{item.verdict}</Pill></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><Stat label="NOAA rainfall" value={mm(item.noaaMm ? Number(item.noaaMm) : null)} /><p className="sky-mono break-all text-[10px] leading-relaxed text-[var(--faint)]">Source hash: {item.sourceHash}</p></div></Card>)}</div>}</section>;
}
