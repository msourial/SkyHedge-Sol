...
/** Final NOAA evidence package for the first activation market. */
export default async function handler(req: IncomingMessage, res: JsonResponse) {
  ...
  if (!process.env.NOAA_TOKEN) {
    return res.status(503).json({ error: "DATA_UNAVAILABLE", message: "NOAA_TOKEN is not configured for this deployment." });
  }
  try {
    ...
    return res.status(503).json({ error: "DATA_UNAVAILABLE", message: "NOAA evidence request failed." });
  }
}
